...
void CMenus::RenderSettingsControls(CUIRect MainView)
{
	...
	for (int i = 0; i < NumTabs; i++)
	{
		...
		if (DoButton_MenuTab(aTabs[i], aTabs[i], IdPage == i, &Button, 0))
			IdPage = i;
	}

	MainView.Margin(10.0f, &MainView);

	if (IdPage == 0) // Movement
	{
		CUIRect Label;
		MainView.HSplitTop(20.0f, &Button, &Temp);
		Button.VSplitLeft(150.0f, &Label, &Button);
		...
		Temp.HSplitTop(20.0f, 0, &Temp);
		UiDoGetButtons(0, 5, Temp);
	}
	else if (IdPage == 1) // Weapon
	{
		//MainView.HSplitTop(14.0f + 5.0f + 10.0f, 0, &MainView);
		UiDoGetButtons(5, 12, MainView);
	}
	else if (IdPage == 2) // Voting
	{
		UiDoGetButtons(12, 14, MainView);
	}
	else if (IdPage == 3) // Chat
	{
		UiDoGetButtons(14, 17, MainView);
	}
	else if (IdPage == 4) // Miscellaneous
	{
		UiDoGetButtons(17, 26, MainView);
	}
...
{
	CUIRect Button, Temp, TabBar, RestartWarning;
	...
	RenderTools()->DrawUIRect(&MainView, ms_ColorTabbarActive, CUI::CORNER_ALL, 10.0f);
	MainView.HSplitBottom(15.0f, 0, &RestartWarning);
	RestartWarning.y += 15.0f;
	RestartWarning.Margin(2.0f, &RestartWarning);

	MainView.HSplitTop(26.0f, &TabBar, &MainView);
	for(int i = 0; i < NumTabs; i++)
	{
		TabBar.VSplitLeft(MainView.w / NumTabs, &Button, &TabBar);
		if (DoButton_MenuTab(aTabs[i], aTabs[i], s_SettingsPage == i, &Button, i ? i == NumTabs - 1 ? CUI::CORNER_TR : 0 : CUI::CORNER_TL))
			s_SettingsPage = i;
	}
