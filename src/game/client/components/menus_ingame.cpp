...
void CMenus::RenderPlayers(CUIRect MainView)
{
	...
	{
		if(!m_pClient->m_Snap.m_paInfoByTeam[i] || m_pClient->m_Snap.m_paInfoByTeam[i]->m_ClientID == m_pClient->m_Snap.m_LocalClientID)
			continue;
	...
	int OldSelected = -1;
	UiDoListboxStart(&s_ScrollValue, &Options, 25.0f, Localize(""), "", NumPlayers, 1, OldSelected, s_ScrollValue);
	...
	for(int i = 0, Count = 0; i < MAX_CLIENTS; ++i)
	{
		...
		if(Index == m_pClient->m_Snap.m_LocalClientID)
			continue;
		Count++;
		CListboxItem Item = UiDoListboxNextItem(&m_pClient->m_Snap.m_paInfoByTeam[i], OldSelected == i);
		if(!Item.m_Visible)
			continue;

		Item.m_Rect.HSplitTop(28.0f, &ButtonBar, &Options);
		if(Count%2 == 0)
		...
		Button.VSplitLeft((Width-Button.h)/4.0f, 0, &Button);
		Button.VSplitLeft(Button.h, &Button, 0);
		if(DoButton_Toggle(&s_aPlayerIDs[Index][1], m_pClient->m_aClients[Index].m_Friend == 1, &Button, true))
		{
			if(m_pClient->m_aClients[Index].m_Friend == 1)
		...
		Button.VSplitLeft((Width - Button.h) / 4.0f, 0, &Button);
		Button.VSplitLeft(Button.h, &Button, 0);
		if(DoButton_Toggle(&s_aPlayerIDs[Index][2], m_pClient->m_aClients[Index].m_Friend == 2, &Button, true))
		{
			if(m_pClient->m_aClients[Index].m_Friend == 2)
				m_pClient->Friends()->RemoveFriend(m_pClient->m_aClients[Index].m_aName, m_pClient->m_aClients[Index].m_aClan);
			else
			{
				m_pClient->Friends()->RemoveFriend(m_pClient->m_aClients[Index].m_aName, m_pClient->m_aClients[Index].m_aClan);
				m_pClient->Friends()->AddEnemy(m_pClient->m_aClients[Index].m_aName, m_pClient->m_aClients[Index].m_aClan);
			}
		}
	}
