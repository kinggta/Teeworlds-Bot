...
static const char *m_aQuitLabels[] = {	"Everytime you quit, a Tee will die in a painful way! :(",
		"You want to quit NOW?! Why don't you stay a bit and play..", 
		"That noob who got pwned by you in your last match said you suck. Better go back and kill him!" };

struct CMainMenuButton
{
	void *pID;
	CUIRect Rect;
};
...
public:
	...
	int DoButton_CheckBox_Common(const void *pID, const char *pText, const char *pBoxText, const CUIRect *pRect, const char *pTip = NULL);
...
private:
	...
	*/
	...
	void RenderInfo(CUIRect MainView);
	void RenderNews(CUIRect MainView);
	void RenderServer(CUIRect MainView);
	...
	// found in menus_13x37.cpp
	int m_XPage;
	void Render13x37Identity(CUIRect MainView);
	void Render13x37Econ(CUIRect MainView);
	void Render13x37Extras(CUIRect MainView);
	void Render13x37Esp(CUIRect MainView);
	void Render13x37Tee(CUIRect MainView);
	void Render13x37Bots(CUIRect MainView);
	void Render13x37Dummy(CUIRect MainView);
