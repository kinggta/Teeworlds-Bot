...
void CMenus::RenderServerbrowserServerList(CUIRect View)
{
	...
	RenderTools()->DrawUIRect(&Headers, vec4(1,1,1,0.25f), 0, 5.0f);
	...
	// users misses it
	{
		...
		if(m_ActivePage == PAGE_BROWSER && m_BrowserPage == BROWSER_INTERNET && ServerBrowser()->IsRefreshingMasters())
...
void CMenus::RenderServerbrowserServerDetail(CUIRect View)
{
	...
	if(pSelectedServer)
	{
		...
		int OldSelected = -1;
		UiDoListboxStart(&s_ScrollValue, &ServerScoreBoard, 25.0f, Localize("Scoreboard"), "", pSelectedServer->m_NumClients, 1, OldSelected, s_ScrollValue, FontSize+2.0f);

		for (int i = 0; i < pSelectedServer->m_NumClients; i++)
		{
			CUIRect Name, Clan, Score, Flag;

			CListboxItem Item = UiDoListboxNextItem(&pSelectedServer->m_aClients[i], OldSelected == i);
			if(!Item.m_Visible)
				continue;

			Item.m_Rect.HSplitTop(25.0f, &Name, &ServerScoreBoard);
		...
		if(OldSelected != NewSelected && pSelectedServer)
		{
			if(pSelectedServer->m_aClients[NewSelected].m_FriendState == IFriends::FRIEND_PLAYER)
				m_pClient->Friends()->RemoveFriend(pSelectedServer->m_aClients[NewSelected].m_aName, pSelectedServer->m_aClients[NewSelected].m_aClan);
			else
				m_pClient->Friends()->AddFriend(pSelectedServer->m_aClients[NewSelected].m_aName, pSelectedServer->m_aClients[NewSelected].m_aClan);
			FriendlistOnUpdate();
			Client()->ServerBrowserUpdate();
		}
	}
...
void CMenus::RenderServerbrowser(CUIRect MainView)
{
	...
	*/
	...
	CUIRect ServerList, ToolBox, StatusBox, PlayerBox, TabBar, ListBar, Button;

	// background
	RenderTools()->DrawUIRect(&MainView, ms_ColorTabbarActive, CUI::CORNER_ALL, 10.0f);
	MainView.Margin(10.0f, &MainView);

	MainView.VSplitRight(205.0f, 0, &ToolBox);
	MainView.HSplitTop(20.0f, &ListBar, &MainView);
	ListBar.VSplitRight(210.0f, &ListBar, 0);
	float w = ListBar.w * 0.25f;
	//ListBar.VMargin(20.0f, &ListBar);
	ListBar.VSplitLeft(w, &Button, &ListBar);

	

	static int s_InternetButton=0;
	if(DoButton_MenuTab(&s_InternetButton, Localize("Internet"), m_BrowserPage==BROWSER_INTERNET, &Button, CUI::CORNER_TL))
	{
	...
	if(DoButton_MenuTab(&s_LanButton, Localize("LAN"), m_BrowserPage==BROWSER_LAN, &Button, 0))
	{
		ServerBrowser()->Refresh(IServerBrowser::TYPE_LAN);
		m_BrowserPage = BROWSER_LAN;
	}

	ListBar.VSplitLeft(w, &Button, &ListBar);
	static int s_RecentButton=0;
	if(DoButton_MenuTab(&s_RecentButton, Localize("Recent"), m_BrowserPage==BROWSER_RECENT, &Button, 0))
	{
		ServerBrowser()->Refresh(IServerBrowser::TYPE_RECENT);
		m_BrowserPage = BROWSER_RECENT;
	}

	ListBar.VSplitLeft(w, &Button, &ListBar);
	static int s_FavoritesButton=0;
	if(DoButton_MenuTab(&s_FavoritesButton, Localize("Favorites"), m_BrowserPage==BROWSER_FAVORITES, &Button, CUI::CORNER_TR))
	{
		ServerBrowser()->Refresh(IServerBrowser::TYPE_FAVORITES);
		m_BrowserPage = BROWSER_FAVORITES;
	}


	// create server list, status box, tab bar and tool box area
	MainView.VSplitRight(205.0f, &ServerList, 0);
	...
	// tool box
	{
		...
		ToolBox.HSplitTop(20.0f, &TabBar, &ToolBox);
		TabBar.VSplitLeft(w, &TabButton, &TabBar);
		static int s_FiltersTab = 0;
		if (DoButton_MenuTab(&s_FiltersTab, Localize("Filter"), ToolboxPage==0, &TabButton, CUI::CORNER_TL))
			ToolboxPage = 0;
		
		TabBar.VSplitLeft(w, &TabButton, &TabBar);
		static int s_InfoTab = 0;
		if (DoButton_MenuTab(&s_InfoTab, Localize("Info"), ToolboxPage==1, &TabButton, 0))
			ToolboxPage = 1;

		TabBar.VSplitLeft(w, &TabButton, &TabBar);
		static int s_FriendsTab = 0;
		if (DoButton_MenuTab(&s_FriendsTab, Localize("Friends"), ToolboxPage==2, &TabButton, CUI::CORNER_TR))
			ToolboxPage = 2;
	...
	// status box
	{
		CUIRect Button, ButtonArea, Queue;
		float Width = TextRender()->TextWidth(0, 14, "XXX.XXX.XXX.XXX:XXXXX", -1);
		StatusBox.HSplitTop(5.0f, 0, &StatusBox);

		// version note
		StatusBox.HSplitBottom(15.0f, &StatusBox, &Button);
		
		if (Client()->GetQueueActive())
		{
			char aBuf[128];
			char aAddr[64];
			float Seconds = (Client()->GetQueueTime() - time_get()) / (float)time_freq();
			net_addr_str(&Client()->GetQueueAddress(), aAddr, sizeof(aAddr), true);
			
			str_format(aBuf, sizeof(aBuf), Localize("Autoreconnecting to %s in %.1f seconds"), aAddr, Seconds);
		...
		Width = TextRender()->TextWidth(0, 14, Localize("Connect"), -1);
		ButtonArea.VSplitLeft(10.0f, 0, &ButtonArea);
		ButtonArea.VSplitLeft(Width*2, &ButtonArea, &Queue);
		...
		{
			if(m_BrowserPage == BROWSER_INTERNET)
				ServerBrowser()->Refresh(IServerBrowser::TYPE_INTERNET);
			else if(m_BrowserPage == BROWSER_LAN)
				ServerBrowser()->Refresh(IServerBrowser::TYPE_LAN);
			else if(m_BrowserPage == BROWSER_FAVORITES)
				ServerBrowser()->Refresh(IServerBrowser::TYPE_FAVORITES);
		}
		...
		//ButtonArea.HSplitTop(20.0f, &Button, &ButtonArea);
		//Button.VMargin(2.0f, &Button);
		//static int s_QuickRefreshButton = 0;
		//if(DoButton_Menu(&s_QuickRefreshButton, Localize("Quick Refresh"), 0, &Button))
		//{
		//	if(m_BrowserPage == BROWSER_INTERNET)
		//		ServerBrowser()->QuickRefresh(IServerBrowser::TYPE_INTERNET);
		//	else if(m_BrowserPage == BROWSER_LAN)
		//		ServerBrowser()->QuickRefresh(IServerBrowser::TYPE_LAN);
		//	else if(m_BrowserPage == BROWSER_FAVORITES)
		//		ServerBrowser()->QuickRefresh(IServerBrowser::TYPE_FAVORITES);
		//}
...
{
	pfnCallback(pResult, pCallbackUserData);
	if(pResult->NumArguments() && ((CMenus *)pUserData)->m_BrowserPage == BROWSER_FAVORITES && ((CMenus *)pUserData)->Client()->State() == IClient::STATE_OFFLINE)
