...
void CGameClient::OnNewSnapshot()
{
	...
	for(int i = 0; i < MAX_CLIENTS; ++i)
	{
		if(i == m_Snap.m_LocalClientID || !m_Snap.m_paPlayerInfos[i] || !Friends()->IsFriend(m_aClients[i].m_aName, m_aClients[i].m_aClan, true))
			m_aClients[i].m_Friend = 0;
		else
			m_aClients[i].m_Friend = Friends()->IsFriend(m_aClients[i].m_aName, m_aClients[i].m_aClan, true);
	}
	...
	if (!m_64PlayerMsgSent[0] && m_Snap.m_pLocalInfo)
	{
		CMsgPacker Msg(NETMSGTYPE_CL_IS64PLAYER);
		Msg.AddInt(1); //1 for DDRace
		Client()->SendMsg(&Msg, MSGFLAG_VITAL);
		m_64PlayerMsgSent[0] = true;
	}

	// no need to send, default settings
	if(!(m_ShowOthers[0] == -1 && g_Config.m_XShowOthers))
	{
		CNetMsg_Cl_ShowOthers Msg;
		Msg.m_Show = g_Config.m_XShowOthers;
		Client()->SendPackMsg(&Msg, MSGFLAG_VITAL);
	}

	// update state
	m_ShowOthers[0] = g_Config.m_XShowOthers;
