...
{
	pObjCore->m_X = xround(m_Pos.x);
	pObjCore->m_Y = xround(m_Pos.y);

	pObjCore->m_VelX = xround(m_Vel.x*256.0f);
	pObjCore->m_VelY = xround(m_Vel.y*256.0f);
	pObjCore->m_HookState = m_HookState;
	pObjCore->m_HookTick = m_HookTick;
	pObjCore->m_HookX = xround(m_HookPos.x);
	pObjCore->m_HookY = xround(m_HookPos.y);
	pObjCore->m_HookDx = xround(m_HookDir.x*256.0f);
	pObjCore->m_HookDy = xround(m_HookDir.y*256.0f);
