...
void CServerBrowser::QuickRefresh(int Type)
{
	if(IsRefreshing())
		return;
	m_CurQuickRefresh = 0;
...
void CServerBrowser::Update(bool ForceResort)
{
	...
	if(m_NeedRefresh == 1 && !m_pMasterServer->IsRefreshing())
	{
		NETADDR Addr;
		CNetChunk Packet;
		int i;
	...
	if(m_NeedRefresh == 2 && !m_pMasterServer->IsRefreshing() && !IsRefreshing())
	{
		//dbg_msg("lol", "%d, %d %d", m_NeedRefresh, m_pMasterServer->IsRefreshing(), IsRefreshing());
		Count = 0;
		for(; m_CurQuickRefresh < m_NumServers; m_CurQuickRefresh++)
		{
			CServerEntry *pEntry = m_ppServerlist[m_CurQuickRefresh];
			if(!pEntry) // no more entries
			{
				m_NeedRefresh = 0;
				break;
			...
			//dbg_msg("req", "from %d.%d.%d.%d:%d", pEntry->m_Addr.ip[0], pEntry->m_Addr.ip[1], pEntry->m_Addr.ip[2], pEntry->m_Addr.ip[3], pEntry->m_Addr.port);

			// no more then 10 concurrent requests
			if(Count == g_Config.m_BrMaxRequests)
				break;

			//if(pEntry->m_RequestTime == 0) //TODO: implement this
				RequestImpl(pEntry->m_Addr, pEntry);
