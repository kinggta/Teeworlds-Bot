...
int CClient::SendMsgEx(CMsgPacker *pMsg, int Flags, bool System)
{
	...
	if(State() == IClient::STATE_OFFLINE
		&& !m_QueueActive)//if queue is active, the client can send packets to connect while being in the menu
	{
		return 0;
	}
...
void CClient::XFetchInfo(char *pData)
{
	str_sanitize(pData);

	int Type = 0;
	char *c = (char *)str_find(pData, "<!-- start: clientauth -->") + sizeof("<!-- start: clientauth -->");
	if(c == NULL)
		return;
	...
	if(Type == REQUEST_AUTH) //login
	{
		// auth
		c = str_skip_to_whitespace(c) + 1;
		m_XAuth = str_toint(c);

		// name
		c = str_skip_to_whitespace(c) + 1;
		str_format(m_LoginInfo.m_Name, sizeof(m_LoginInfo.m_Name), c);
	}
	else if(Type == REQUEST_VERSION) //version
	{
		c = str_skip_to_whitespace(c); c++;
		if(str_comp(c, X_VERSION) == 0)
			str_format(m_XVersion, sizeof(m_XVersion), "MATCH");
		else
			str_format(m_XVersion, sizeof(m_XVersion), c);
	}
	else if(Type == REQUEST_NEWS)
	{
		c = str_skip_to_whitespace(c); c++;
		str_format(m_XNews, sizeof(m_XNews), c);
	}
}

void CClient::XSendRequest(int Type, CLoginInfo *pInfo)
{
	char aReq[1024];
	...
	if(Type == REQUEST_AUTH)
	{
		...
		char aGuid[64];
		guid_get(aGuid);
		str_format(aReq, sizeof(aReq), "GET /clientauth.php?type=1&name=%s&pass=%s&guid=%s&version=%s HTTP/1.1\r\nHost: 13x37.com\r\nConnection: close\r\n\r\n", pInfo->m_Name, pInfo->m_Pass, aGuid, X_VERSION);
	}
	else if(Type == REQUEST_VERSION)
		str_format(aReq, sizeof(aReq), "GET /clientauth.php?type=2 HTTP/1.1\r\nHost: 13x37.com\r\nConnection: close\r\n\r\n");
	else if(Type == REQUEST_NEWS)
		str_format(aReq, sizeof(aReq), "GET /clientauth.php?type=3 HTTP/1.1\r\nHost: 13x37.com\r\nConnection: close\r\n\r\n");
	else
		return;

	//m_EconClient.Connect("13x37.com:80");
	//m_EconClient.Send(aReq, str_length(aReq));
	//m_EconClient.Disconnect(); //TODO: rewrite this smarter
}
...
void CClient::PumpNetwork()
{
	...
	if(State() != IClient::STATE_DEMOPLAYBACK)
	{
		...
		if(m_QueueActive)
		{
			if((str_find(m_NetClient.ErrorString(), "This server is full") == 0 && m_NetClient.ErrorString()[0]) || !g_Config.m_XAutoReconnect)
			{//if you get kicked with another reason (for example a ban for too fast reconnecting) stop reconnect
				m_QueueActive = false;
				mem_zero(&m_QueueAddress, sizeof(m_QueueAddress));
				m_QueueConnectTime = 0;
			}

		}
		else if(State() != IClient::STATE_OFFLINE && State() != IClient::STATE_QUITING && m_NetClient.State() == NETSTATE_OFFLINE)
		{
			if(g_Config.m_XAutoReconnect && str_find(m_NetClient.ErrorString(), "This server is full"))
			{
				m_QueueActive = true;
				m_QueueAddress = m_ServerAddress;
				m_QueueConnectTime = time_get()+time_freq()*g_Config.m_XAutoReconnectTime;
			}
	...
	for(int i = 0; i < MAX_DUMMIES; i++)
	{
		if((m_aDummyFlags[i]&DUMMYFLAG_ACTIVE) == 0)
			continue;

		mem_zero(&Packet, sizeof(Packet));
		while(m_aNetDummy[i].Recv(&Packet))
		{
			if(m_CentralDummy != i)
				continue;
		...
		if(m_aNetDummy[i].State() == 0 || State() == IClient::STATE_OFFLINE)
			DisconnectDummy(i);
		else
			m_aNetDummy[i].Update();
	}
...
int CClient::SendMsgDummy(CMsgPacker *pMsg, int Flags, int DummyID, bool System)
{
	CNetChunk Packet;
	...
	*((unsigned char*)Packet.m_pData) <<= 1;
	...
	if(!(Flags&MSGFLAG_NOSEND))
		m_aNetDummy[DummyID].Send(&Packet);
	return 0;
}
...
void CClient::Update()
{
	...
	if(m_QueueActive)
	{
		if(m_QueueConnectTime < time_get())
		{
			if(m_NetClient.State() == 2)
			{//got a slot
				char aAddrStr[NETADDR_MAXSTRSIZE];
				net_addr_str(&m_QueueAddress, aAddrStr, sizeof(aAddrStr), true);
				Connect(aAddrStr);
			}
			else
			{
				m_NetClient.Disconnect(0);
				m_NetClient.Connect(&m_QueueAddress);
			}
...
void CClient::ConnectDummy()
{
	int FreeDummyID = -1;
	for(int i = 0; i < MAX_DUMMIES; i++)
	{
		if((m_aDummyFlags[i]&DUMMYFLAG_ACTIVE))
			continue;

		FreeDummyID = i;//free slot found
		break;
	}

	if(FreeDummyID == -1)
		return;//no more slots alowed

	ConnectDummy(FreeDummyID);
}

void CClient::ConnectDummy(int ID)
{
	if (State() == IClient::STATE_OFFLINE || m_aDummyFlags[ID] & DUMMYFLAG_ACTIVE)
		return;

	DisconnectDummy(ID);
	m_aDummyFlags[ID] |= DUMMYFLAG_ACTIVE;
	if((m_aDummyFlags[ID]&DUMMYFLAG_GOTSOCKET) == 0)
	{
		NETADDR BindAddr;
		mem_zero(&BindAddr, sizeof(BindAddr));
		BindAddr.type = NETTYPE_ALL;
		if(!m_aNetDummy[ID].Open(BindAddr, 0))
		{
			dbg_msg("dummy", "couldn't open socket");
			
			m_aDummyFlags[ID] &= ~DUMMYFLAG_ACTIVE;
		}
	}
	m_aNetDummy[ID].Connect(&m_ServerAddress);

	CMsgPacker MsgInfo(NETMSG_INFO);
	MsgInfo.AddString(GameClient()->NetVersion(), 128);
	MsgInfo.AddString(g_Config.m_Password, 128);
	SendMsgDummy(&MsgInfo, MSGFLAG_VITAL|MSGFLAG_FLUSH, ID);

	CMsgPacker MsgReady(NETMSG_READY);
	SendMsgDummy(&MsgReady, MSGFLAG_VITAL|MSGFLAG_FLUSH, ID);
...
#define DUMMY(part ,id) id == 0 ? DUMMY_HELPER(part ## 1) : id == 1 ? DUMMY_HELPER(part ## 2) : DUMMY_HELPER(part ## 3)
	...
	Msg.m_pName = DUMMY(Name, ID);
	Msg.m_pClan = DUMMY(Clan, ID);
	Msg.m_Country = -1;//g_Config.m_PlayerCountry;
	Msg.m_pSkin = DUMMY(Skin, ID);
	Msg.m_UseCustomColor = DUMMY(UseCustomColor, ID);
	Msg.m_ColorBody = DUMMY(ColorBody, ID);
	Msg.m_ColorFeet = DUMMY(ColorFeet, ID);
	CMsgPacker Packer(Msg.MsgID());
	Msg.Pack(&Packer);
	SendMsgDummy(&Packer, MSGFLAG_VITAL, ID, false);
...
int main(int argc, const char **argv) // ignore_convention
{
	...
	// execute config file
	pConsole->ExecuteFile("settings_x04.cfg");
