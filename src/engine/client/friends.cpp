...
void CFriends::ConAddFriend(IConsole::IResult *pResult, void *pUserData)
{
	...
	int Enemy = pResult->GetInteger(2);
	if(Enemy)
		pSelf->AddEnemy(pResult->GetString(0), pResult->GetString(1));
	else
...
void CFriends::Init()
{
	...
	if(pConsole)
	{
		pConsole->Register("add_friend", "ss?i", CFGFLAG_CLIENT, ConAddFriend, this, "Add a friend");
...
int CFriends::GetFriendState(const char *pName, const char *pClan) const
{
	...
	{
		if(m_aFriends[i].m_ClanHash == ClanHash && !m_aFriends[i].m_Enemy)
		{
...
int CFriends::IsFriend(const char *pName, const char *pClan, bool PlayersOnly) const
{
	unsigned NameHash = str_quickhash(pName);
	unsigned ClanHash = str_quickhash(pClan);
	for(int i = 0; i < m_NumFriends; ++i)
...
void CFriends::AddEnemy(const char *pName, const char *pClan)
{
	...
	m_aFriends[m_NumFriends].m_Enemy = true;
	++m_NumFriends;
}
...
void CFriends::ConfigSaveCallback(IConfig *pConfig, void *pUserData)
{
	...
	for(int i = 0; i < pSelf->m_NumFriends; ++i)
	{
		...
		*pDst++ = 0;
		...
		if(pSelf->m_aFriends[i].m_Enemy)
			str_append(aBuf, " 1", sizeof(aBuf));
